...
(function () {
  ...
  var ST_CLASS = {
    pending: 'hold', published: 'done', rejected: 'canceled',
    hidden: 'canceled', expired: 'received',
  };
      ...
      onMount: function (mount) {
        ...
        if (approve) {
          approve.addEventListener('click', function () {
            if (!paid.checked &&
              !window.confirm('등록비 입금이 확인되지 않았습니다. 그래도 게시하시겠습니까?')) return;
            feeStep()
  ...
  A.register('listings', {
    ...
    badge: function (state) {
      return (state.listings || []).filter(function (r) { return r.status === 'pending'; }).length;
    },
    ...
    render: function (root, state, ctx) {
      ...
      var counts = {
        ...
        unpaid: all.filter(function (r) { return !(r.fee || {}).paid && r.status !== 'rejected'; }).length,
      ...
      root.innerHTML =
        '<div class="adm-card" style="margin-bottom:18px">' +
          ...
          '<p class="adm-card-lead">등록자가 홈페이지에서 직접 올린 매물이 <strong>승인 대기</strong>로 들어옵니다. ' +
            '글을 열어 <strong>권리 증빙 서류</strong>(등기부등본 · 임대차계약서 · 위임장)를 확인하고, ' +
            '등록비 ' + money(db.LISTING_FEE) + ' 입금을 표시한 뒤 [게시하기] 를 누르면 게시판에 공개됩니다. ' +
            '서류가 맞지 않으면 사유를 적어 반려해 주세요 — 그 사유가 등록자에게 그대로 보입니다.</p>' +
          '<p class="adm-card-lead">기본 게시 기간은 ' + db.LISTING_DAYS + '일이고, 기간이 지나면 목록에서 자동으로 내려갑니다. ' +
            '중개 · 조건 협의는 센터 업무가 아닙니다. 문의 전화는 등록자에게 직접 갑니다.</p>' +
        '</div>' +

        '<div class="adm-stats">' +
          '<div class="adm-stat is-accent"><strong>' + counts.pending + '</strong><span>승인 대기</span></div>' +
          '<div class="adm-stat"><strong>' + counts.published + '</strong><span>게시중</span></div>' +
          '<div class="adm-stat"><strong>' + counts.unpaid + '</strong><span>등록비 미확인</span></div>' +
        ...
        '<div class="adm-toolbar">' +
          ...
          '<select id="lsFilter">' +
            '<option value="pending"' + (filter === 'pending' ? ' selected' : '') + '>승인 대기만</option>' +
        ...
        (rows.length
          ? rows.map(function (r) {
              ...
              return '<tr data-id="' + h(r.id) + '" class="is-click">' +
                ...
                '<td>' + h(v.kindLabel) + '<span class="sub">' + h(db.listingPrice(r)) + '</span></td>' +
                '<td>' + h(v.holderLabel) +
          ...
          : A.emptyRow(6,
              filter === 'pending' ? '승인 대기 중인 매물이 없습니다' : '매물이 없습니다',
      ...
      ctx.actions.querySelector('#lsCsv').addEventListener('click', function () {
        A.downloadCsv(
          'caps-매물게시판.csv',
          ['제목', '종류', '용도', '지역', '위치', '금액', '면적', '층', '등록자 입장', '제출 서류',
            '등록 계정', '연락처', '상태', '등록비', '등록일', '게시일', '게시 종료', '사유'],
          rows.map(function (r) {
            var v = db.listingView(r);
            var u = userOf(state, r.userId);
            return [r.title, v.kindLabel, v.useLabel, r.region, r.addressRough,
              db.listingPrice(r), r.area, r.floor, v.holderLabel, v.proofLabel,
              u ? u.email : (r.userEmail || ''), db.formatPhone(r.contactPhone), v.label,
              (r.fee || {}).paid ? '확인' : '미확인',
