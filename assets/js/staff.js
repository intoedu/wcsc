...
(function () {
  ...
  phone.addEventListener('blur', function () {
    if (phone.value.trim()) phone.value = db.formatPhone(phone.value);
  });
  ...
  signupForm.addEventListener('submit', function (e) {
    ...
    db.auth.signUp({
      email: document.getElementById('spSuEmail').value,
      password: document.getElementById('spSuPw').value,
      name: document.getElementById('spName').value,
      phone: db.formatPhone(document.getElementById('spPhone').value),
      staffRequest: true,   // 이 페이지의 가입은 항상 직원 신청입니다
    }).catch(function (ex) {
      justSignedUp = false;
      err.textContent = ex.message;
      err.hidden = false;
